...
- `@threadlabs/looma@0.1.28` is public under the non-default `candidate` tag with
...
- Candidate `0.1.28` was published from commit
  `9991671f505a066e3a0de2a3e02dcbaf362f31fc` by workflow run
  `34710572669` after exact-main CI passed, then its public registry bytes passed
  Knit's full static and browser qualification gates.
...
1. Keep Candidate `0.1.28` and its viewport-safe, single-surface overlays
   qualified against Knit's exact public registry dependency.
