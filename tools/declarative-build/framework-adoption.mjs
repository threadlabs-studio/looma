...
export function preserveVueSlotRegions(source) {
  ...
  const withNamedRegions = source.replace(
    /<slot name="([^"]+)"><\/slot>/g,
    '<span slot="$1" style="display: contents"><slot name="$1"></slot></span>',
  );
  ...
  // mark their default region with the native slot="" so hydration assigns it to the default slot.
  if (slotCount <= 1) return withNamedRegions;
  return withNamedRegions.replace(
    /<slot><\/slot>/g,
    '<span slot="" style="display: contents"><slot></slot></span>',
  );
}
