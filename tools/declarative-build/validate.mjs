...
const packageDirectories = ["core", "layout", "editor"].map((name) =>
  join(REPOSITORY, "packages", name, "src", "declarative", "components"));
const components = (await Promise.all(packageDirectories.map(async (directory) =>
  (await readdir(directory))
    .filter((name) => name.endsWith(".html"))
    .map((name) => ({ tag: name.slice(0, -5), directory }))))).flat();
...
console.log(`Validated ${components.length} canonical HTML Next definitions in ${result.passes} lowering passes.`);
} finally {
  await browser.close();
}
