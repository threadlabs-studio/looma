...
test("Candidate changelog records the exact R1 support boundary without claiming remote completion", () => {
  ...
  assert.match(changelog, /^## v0\.1\.13 Candidate$/m);
...
test("release record downloads and validates the exact Candidate and promotion evidence", () => {
  ...
  assert.match(
    releaseRecordJob,
    /name: looma-0\.1\.13-candidate[\s\S]*?path: \.release\/artifacts\/[\s\S]*?run-id: \$\{\{ inputs\.candidate_workflow_run_id \}\}/
  );
  assert.match(
    releaseRecordJob,
    /name: looma-0\.1\.13-registry-promotion-evidence[\s\S]*?path: \.release\/evidence\/[\s\S]*?run-id: \$\{\{ github\.run_id \}\}/
  );
