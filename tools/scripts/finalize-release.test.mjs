...
function manifest() {
  return {
    ...
    packages: packageNames.map((name, publishIndex) => ({
      ...
      name,
      version: "0.1.10",
      tarball: `${name.split("/")[1]}-0.1.10.tgz`,
  ...
  return {
    ...
    verification: {
      ...
      packages: releaseManifest.packages.map((entry) => ({
        ...
        version: entry.version,
        integrity: `sha512-${entry.publishIndex}`,
        distTags: { candidate: "0.1.10", latest: "0.1.10" },
...
test("validates one exact promoted graph and derives the immutable tag", () => {
  ...
  assert.equal(record.tag, "v0.1.10");
  assert.equal(record.title, "Looma v0.1.10 Candidate");
