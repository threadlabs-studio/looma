...
function releaseManifest(overrides = {}) {
  return {
    ...
    packages: releaseNames.map((name, publishIndex) => ({
      ...
      name,
      version: "0.3.0",
      tarball: `${name.replace(/^@/, "").replace("/", "-")}-0.3.0.tgz`,
...
function sourcePackages() {
  return Object.fromEntries(releaseNames.map((name) => [name, { name, version: "0.3.0" }]));
}

function publicLockfile(version = "0.3.0") {
...
test("accepts an eligible current manifest and derives exact public dependencies", () => {
  ...
  assert.deepEqual(
    ...
    Object.fromEntries(releaseNames.map((name) => [name, "0.3.0"]))
  );
  ...
  assert.deepEqual(resolutions.map(({ name, version }) => ({ name, version })),
    releaseNames.map((name) => ({ name, version: "0.3.0" })));

  const installed = releaseNames.map((name) => ({ name, version: "0.3.0" }));
...
test("rejects local Looma lockfile resolutions", () => {
  const lockfile = publicLockfile().replace(
    "specifier: 0.3.0\n        version: 0.3.0",
    "specifier: file:../../../.release/artifacts/looma-0.3.0.tgz\n        version: file:../../../.release/artifacts/looma-0.3.0.tgz"
  );
  assert.throws(
...
test("rejects exact-version drift in the lockfile and installed graph", () => {
  ...
  assert.throws(
    ...
    /@threadlabs\/looma installed 0\.1\.2 instead of 0\.3\.0/
  );
});
...
test("builds deterministic evidence from manifest identities and public resolutions", () => {
  ...
  assert.deepEqual(evidence.manifest.packages[0], {
    ...
    version: "0.3.0",
    tarball: "threadlabs-looma-0.3.0.tgz",
    sha256: "1".repeat(64)
  });
