...
function releaseManifest(overrides = {}) {
  return {
    ...
    packages: releaseNames.map((name, publishIndex) => ({
      ...
      name,
      version: "0.1.19",
      tarball: `${name.replace(/^@/, "").replace("/", "-")}-0.1.19.tgz`,
...
function sourcePackages() {
  return Object.fromEntries(releaseNames.map((name) => [name, { name, version: "0.1.19" }]));
}

function publicLockfile(version = "0.1.19") {
...
test("accepts an eligible current manifest and derives exact public dependencies", () => {
  ...
  assert.deepEqual(
    ...
    Object.fromEntries(releaseNames.map((name) => [name, "0.1.19"]))
  );
  ...
  assert.deepEqual(resolutions.map(({ name, version }) => ({ name, version })),
    releaseNames.map((name) => ({ name, version: "0.1.19" })));

  const installed = releaseNames.map((name) => ({ name, version: "0.1.19" }));
...
test("rejects local Looma lockfile resolutions", () => {
  const lockfile = publicLockfile().replace(
    "specifier: 0.1.19\n        version: 0.1.19",
    "specifier: file:../../../.release/artifacts/looma-0.1.19.tgz\n        version: file:../../../.release/artifacts/looma-0.1.19.tgz"
  );
  assert.throws(
...
test("rejects exact-version drift in the lockfile and installed graph", () => {
  ...
  assert.throws(
    ...
    /@threadlabs\/looma installed 0\.1\.2 instead of 0\.1\.19/
  );
});
...
test("builds deterministic evidence from manifest identities and public resolutions", () => {
  ...
  assert.deepEqual(evidence.manifest.packages[0], {
    ...
    version: "0.1.19",
    tarball: "threadlabs-looma-0.1.19.tgz",
    sha256: "1".repeat(64)
  });
