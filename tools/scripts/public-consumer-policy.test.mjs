...
function releaseManifest(overrides = {}) {
  return {
    ...
    packages: releaseNames.map((name, publishIndex) => ({
      ...
      name,
      version: "0.2.9",
      tarball: `${name.replace(/^@/, "").replace("/", "-")}-0.2.9.tgz`,
...
function sourcePackages() {
  return Object.fromEntries(releaseNames.map((name) => [name, { name, version: "0.2.9" }]));
}

function publicLockfile(version = "0.2.9") {
...
test("accepts an eligible current manifest and derives exact public dependencies", () => {
  ...
  assert.deepEqual(
    ...
    Object.fromEntries(releaseNames.map((name) => [name, "0.2.9"]))
  );
  ...
  assert.deepEqual(resolutions.map(({ name, version }) => ({ name, version })),
    releaseNames.map((name) => ({ name, version: "0.2.9" })));

  const installed = releaseNames.map((name) => ({ name, version: "0.2.9" }));
...
test("rejects local Looma lockfile resolutions", () => {
  const lockfile = publicLockfile().replace(
    "specifier: 0.2.9\n        version: 0.2.9",
    "specifier: file:../../../.release/artifacts/looma-0.2.9.tgz\n        version: file:../../../.release/artifacts/looma-0.2.9.tgz"
  );
  assert.throws(
...
test("rejects exact-version drift in the lockfile and installed graph", () => {
  ...
  assert.throws(
    ...
    /@threadlabs\/looma installed 0\.1\.2 instead of 0\.2\.9/
  );
});
...
test("builds deterministic evidence from manifest identities and public resolutions", () => {
  ...
  assert.deepEqual(evidence.manifest.packages[0], {
    ...
    version: "0.2.9",
    tarball: "threadlabs-looma-0.2.9.tgz",
    sha256: "1".repeat(64)
  });
