...
**Import:** `import "@looma/core"` — or use `IconButton` from `@looma/react`, `@looma/vue`, or `@looma/svelte`.
...
import { IconButton } from "@looma/vue";
</script>
...
```tsx
import { IconButton } from "@looma/react";
