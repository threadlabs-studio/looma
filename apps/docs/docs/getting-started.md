...
Looma Release 1 is a Candidate `0.2.17` package for Vue 3 and direct declarative HTML use. It is not Stable yet. React and Svelte adapters in the repository are internal previews and are not published or supported in Release 1.

:::caution Confirm the Candidate tag

These instructions target the exact `@threadlabs/looma@0.2.17` Candidate. Before adopting it, confirm that npm resolves that package at `0.2.17` under the `candidate` dist-tag. Preview documentation can be built before that registry gate; production documentation is published only after the gate passes.
