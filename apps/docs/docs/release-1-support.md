...
Release 1 is a public npm **Candidate `0.2.14`** for applications that want Looma's
...
Candidate availability is defined by the npm `candidate` dist-tag resolving `@threadlabs/looma@0.2.14`. A preview build describes the reviewed contract without asserting registry availability; the production build is deployed only from the same source commit after public Candidate verification.
