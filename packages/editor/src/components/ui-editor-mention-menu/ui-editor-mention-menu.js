...
export default function controller(host) {
  ...
  const onClick = (event) => { const option = event.target.closest?.("[data-index]"); if (option) host.dispatch("select", { index: Number(option.dataset.index) }); };
  const onMouseover = (event) => { const option = event.target.closest?.("[data-index]"); const index = Number(option?.dataset.index); if (!Number.isFinite(index) || index === selectedIndex) return; selectedIndex = index; syncSelection(); host.dispatch("highlight", { index }); };
