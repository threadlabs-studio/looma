...
Looma's component surface is a framework-neutral declarative graph. Each component definition and
...
Release status: Candidate `0.2.20`, not Stable.
...
Looma lowers the invocation directly to a native `<button>` light-DOM root, then attaches styling,
state synchronization, methods, and declared events. Public modules remain safe to import during
SSR, and no custom-element registry is required.
