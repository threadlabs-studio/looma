...
const explicitProps = (): Record<string, unknown> => {
  ...
  const valueloading = props.loading;
  const valueopen = props.open;
  const valuequery = props.query;
  const valueselectedIndex = props.selectedIndex;
  return { "anchorRect": passed("anchorRect", "anchor-rect") ? valueanchorRect : undefined, "items": passed("items", "items") ? valueitems : undefined, "loading": passed("loading", "loading") ? valueloading : undefined, "open": passed("open", "open") ? valueopen : undefined, "query": passed("query", "query") ? valuequery : undefined, "selectedIndex": passed("selectedIndex", "selected-index") ? valueselectedIndex : undefined };
};
...
onMounted(() => {
  ...
  detach = attachLoomaComponent(root.value, definition, "ui-editor-mention-menu", explicitProps());
