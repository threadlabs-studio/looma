...
watchEffect(() => {
  if (root.value == null) return;
  for (const name of ["accepts","container","disabled","dragType","dropDepth","dropScope","expanded","itemId","label","selected","sortable","subtreeDepth"]) (root.value as unknown as Record<string, unknown>)[name] = props[name as keyof typeof props];
});
onUnmounted(() => {
