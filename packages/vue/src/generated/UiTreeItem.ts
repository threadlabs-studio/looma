...
const _hoisted_1 = ["data-drop-scope", "data-accepts"]
const _hoisted_2 = {
  class: "row",
  part: "row",
...
export default /*@__PURE__*/_defineComponent({
  ...
  props: {
    ...
    expanded: { type: [Boolean, null], required: false, default: false },
...
const explicitProps = (): Record<string, unknown> => {
  const valueaccepts = props.accepts;
  const valuecontainer = props.container;
  const valuedisabled = props.disabled;
  const valuedragType = props.dragType;
  const valuedropDepth = props.dropDepth;
  const valuedropScope = props.dropScope;
  const valueexpanded = props.expanded;
  const valueitemId = props.itemId;
  const valuelabel = props.label;
  const valueselected = props.selected;
  const valuesortable = props.sortable;
  const valuesubtreeDepth = props.subtreeDepth;
  return { "accepts": passed("accepts", "accepts") ? valueaccepts : undefined, "container": passed("container", "container") ? valuecontainer : undefined, "disabled": passed("disabled", "disabled") ? valuedisabled : undefined, "dragType": passed("dragType", "drag-type") ? valuedragType : undefined, "dropDepth": passed("dropDepth", "drop-depth") ? valuedropDepth : undefined, "dropScope": passed("dropScope", "drop-scope") ? valuedropScope : undefined, "expanded": passed("expanded", "expanded") ? valueexpanded : undefined, "itemId": passed("itemId", "item-id") ? valueitemId : undefined, "label": passed("label", "label") ? valuelabel : undefined, "selected": passed("selected", "selected") ? valueselected : undefined, "sortable": passed("sortable", "sortable") ? valuesortable : undefined, "subtreeDepth": passed("subtreeDepth", "subtree-depth") ? valuesubtreeDepth : undefined };
};
...
onMounted(() => {
  ...
  detach = attachLoomaComponent(root.value, definition, "ui-tree-item", explicitProps());
